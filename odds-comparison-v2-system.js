...
class OddsComparisonSystemV2 {
    ...
    async fetchFromAPI(sport) {
        // Connect to backend API for real odds data from The Odds API
        try {
            const backendUrl = window.APP_CONFIG?.API?.BASE_URL || CONFIG?.API?.BASE_URL || 'https://ultimate-sports-ai-backend-production.up.railway.app';
            const url = `${backendUrl}/api/odds/${sport}`;
            console.log(`🔄 Fetching odds from backend: ${url}`);
            ...
            if (!response || !response.ok) {
                console.warn(`⚠️ Odds API response not OK: ${response?.status} ${response?.statusText}`);
                return null;
            }
            
            const data = await response.json();
            console.log(`✅ Successfully fetched odds for ${sport}:`, data);
            return data.odds || data; // Handle both wrapped and unwrapped responses
        } catch (error) {
